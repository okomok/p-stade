...
{
private:
	typedef boost::base_from_member<typename boost::range_value<ForwardRange>::type, 0> space_initializer_t;
	typedef boost::base_from_member<typename boost::range_value<ForwardRange>::type, 1> tab_initializer_t;
...
public:
	explicit tab_unexpand_range(ForwardRange& rng, int tabsize) :
		space_initializer_t(PSTADE_OVEN_DEBUG_SPACE_CH),
		tab_initializer_t(PSTADE_OVEN_DEBUG_TAB_CH),
		super_t(
			iter_t(boost::begin(rng), boost::end(rng), tabsize, space_initializer_t::member, tab_initializer_t::member),
			iter_t(boost::end(rng), boost::end(rng), tabsize, space_initializer_t::member, tab_initializer_t::member)
		)
	{ }
};
