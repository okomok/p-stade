...
private:
	typedef tab_expand_iterator<base_iterator> iter_t;
...
///////////////////////////////////////////////////////////////////////////////
// make_tab_expand_range
//
template< class ForwardRange > inline
tab_expand_range<ForwardRange>
make_tab_expand_range(ForwardRange& rng, int tabsize)
{
	return tab_expand_range<ForwardRange>(rng, tabsize);
}

#if !defined(PSTADE_WORKAROUND_NO_RVALUE_DETECTION)
	template<class ForwardRange > inline
	tab_expand_range<const ForwardRange>
	make_tab_expand_range(const ForwardRange& rng, int tabsize)
	{
		return tab_expand_range<const ForwardRange>(rng, tabsize);
	}
#endif


///////////////////////////////////////////////////////////////////////////////
// tab_expand_range_adaptor
//
struct tab_expanded
{
	tab_expanded(int tabsize) : m_tabsize(tabsize) { }
	int m_tabsize;
};
...
template< class ForwardRange > inline
tab_expand_range<ForwardRange>
operator|(ForwardRange& rng, tab_expanded ad)
{
	return oven::make_tab_expand_range(rng, ad.m_tabsize);
}

#if !defined(PSTADE_WORKAROUND_NO_RVALUE_DETECTION)
	template< class ForwardRange > inline
	tab_expand_range<const ForwardRange>
	operator|(const ForwardRange& rng, tab_expanded ad)
	{
		return oven::make_tab_expand_range(rng, ad.m_tabsize);
	}
#endif
...
template< class ForwardRange > inline
boost::mpl::true_
*boost_foreach_is_lightweight_proxy(pstade::oven::tab_expand_range<ForwardRange> *&, boost::foreach::tag)
{ return 0; }
