...
namespace pstade {
        ...
        lambda_sig
    {
        ...
        NullaryResult // don't call 'boost::result_of', which require 'Derived' to be complete.
        operator()() const
        {
            return static_cast<Derived const&>(*this).template call<
                NullaryResult
            >();
        }
        ...
        template< class A0 >
        typename result<callable(A0&)>::type
        ...
        template< class A0 >
        typename result<callable(A0 const&)>::type
        operator()(A0 const& a0) const
        {
            return static_cast<Derived const&>(*this).template call<
                typename result<callable(PSTADE_CONST(A0)&)>::type
            >(a0);
        }
    ...
    #define PSTADE_call_operator(R, BitSeq) \
        ...
        typename result<callable(BOOST_PP_SEQ_FOR_EACH_I_R(R, PSTADE_arg_type, ~, BitSeq))>::type \
