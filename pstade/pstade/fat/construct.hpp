#ifndef BOOST_PP_IS_ITERATING
#ifndef PSTADE_FAT_CONSTRUCT_HPP
#define PSTADE_FAT_CONSTRUCT_HPP
#include "./detail/prefix.hpp"


// PStade.Fat
//
// Copyright Shunsuke Sogame 2007.
// Distributed under the Boost Software License, Version 1.0.
// (See accompanying file LICENSE_1_0.txt or copy at
// http://www.boost.org/LICENSE_1_0.txt)


#include <boost/mpl/placeholders.hpp>
#include <boost/preprocessor/iteration/iterate.hpp>
#include <boost/preprocessor/repetition/enum_binary_params.hpp>
#include <boost/preprocessor/repetition/enum_params.hpp>
#include <pstade/egg/apply_params.hpp>
#include <pstade/egg/automatic.hpp>
#include <pstade/egg/config.hpp> // PSTADE_EGG_MAX_ARITY
#include <pstade/egg/function.hpp>
#include <pstade/egg/object.hpp>


namespace pstade { namespace fat {


    namespace construct_detail {


        template<class X>
        struct baby
        {
        // 0ary
            typedef X nullary_result_type;

            template<class Result>
            Result call() const
            {
                return Result();
            }

        // 1ary-
            template<class Myself, PSTADE_EGG_APPLY_PARAMS(A)>
            struct apply
            {
                typedef X type;
            };

            #define  BOOST_PP_ITERATION_PARAMS_1 (3, (1, PSTADE_EGG_MAX_ARITY, <pstade/fat/construct.hpp>))
            #include BOOST_PP_ITERATE()
        };


    } // namespace construct_detail


    template<class X>
    struct tp_construct
    {
        typedef
            egg::function< construct_detail::baby<X> >
        type;
    };


    template<class X>
    struct xp_construct :
        tp_construct<X>::type
    { };


    // There is no 'construct' yet. This is usually good enough?
    typedef egg::automatic<tp_construct<boost::mpl::_1>::type>::type op_constructor;
    PSTADE_EGG_OBJECT((op_constructor), constructor) = PSTADE_EGG_AUTOMATIC_INITIALIZER();


} } // namespace pstade::fat


#endif
#else
#define n BOOST_PP_ITERATION()


    template<class Result, BOOST_PP_ENUM_PARAMS(n, class A)>
    Result call(BOOST_PP_ENUM_BINARY_PARAMS(n, A, & a)) const
    {
        return Result(BOOST_PP_ENUM_PARAMS(n, a));
    }


#undef n
#endif
