...
void pstade_unit_test()
{
        ...
        for (int i = 0; i < 500; ++i) {
            {
                ...
                test::single_pass_readable(block(::my_block1()), a);
                std::cout << "my_block1 pass" << std::endl;
            }
            {
                int a[] = { 1,2,3,1,2,3,1,2,3 };
                test::single_pass_readable(block(::my_block2(3)), a);
                std::cout << "my_block2 pass" << std::endl;
            }
            {
                test::emptiness(block(::empty_block()));
    ...
    {
        int c = 1;
        BOOST_FOREACH (int& i, block(::reference_block())) {
            if (c == 1)
    ...
    {
        int c = 1;
        BOOST_FOREACH (int const& i, block(::const_reference_block())) {
            if (c == 1)
