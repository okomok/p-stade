...
void test()
{
...

    {
        ...
        BOOST_CHECK( oven::test_RandomAccess_Readable_Writable(
            pstade::egg::get_c<0>(pstade::egg::pack(rng0, rng1)|zipped|unzipped),
            ans0
        ) );

        BOOST_CHECK( oven::test_RandomAccess_Readable_Writable(
            pstade::egg::get_c<1>(pstade::egg::pack(rng0, rng1)|zipped|unzipped),
            ans1
        ) );
    }
    {
        ...
        BOOST_FOREACH (
            int& i,
            pstade::egg::pack(src0, src1)|zipped|elements_c<1>()
        ) {
            if (i == 4)
                i = 5;
        }
...

    {
        ...
        BOOST_CHECK((
            oven::equals(
                boost::get<0>(pstade::egg::pack(src0, src1)|zipped|unzipped),
                src0
            )
        ));
        ...
        BOOST_CHECK((
            oven::equals(
                boost::get<1>(pstade::egg::pack(src0, src1)|zipped|unzipped),
                src1
            )
        ));
    }
