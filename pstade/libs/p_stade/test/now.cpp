...
struct my_string :
    boost::movable<my_string>
{
    explicit my_string(char const *psz) :
        m_pimpl(new std::string(psz))
    { }

    // non-const lvalue - copy ctor
    BOOST_LVALUE_COPY_CTOR(my_string, (other)
        (: m_pimpl(new std::string(*other.m_pimpl))),
    {
        std::cout << "copy\n";
    })
    ...
    my_string(boost::move_from<my_string> other) :
...
my_string make_string1()
{
    my_string str("hello");
    return str;
}

my_string make_string2()
{
    my_string str("hello");
...
int main()
{
    my_string my1(make_string1());
    my_string my2(make_string2());
}
*/