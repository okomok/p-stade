...
const fs::path find_bin_path(const string& relative)
{
  ...
  {
    bin_path = locate_root / "bin/pstade" / relative; // fixed by MB
    if (!fs::exists(bin_path))
    {
...
//  do_table  ----------------------------------------------------------------//
  ...
  void do_table()
  {
    ...
    relative.erase( 0, locate_root.string().size()+1 ); // fixed by MB
...
int cpp_main( int argc, char * argv[] ) // note name!
{
  ...
  report << "<html>\n"
          ...
          "<table border=\"0\">\n"
          "<tr>\n"
          "<td><img border=\"0\" src=\"../libs/doc/logo.png\" width=\"200\" "
          "height=\"164\"></td>\n"
          "<td>\n"
