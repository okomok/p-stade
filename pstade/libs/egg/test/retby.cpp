...
typedef result_of_retby<T_plus, nullary_meta< ::add_const > >::type T_my_plus;
...
typedef result_of_retby<T_plus, boost::add_const<boost::mpl::_1> >::type T_my_plus2;
...
{
    {
        int x = egg::retby< nullary_meta< ::add_const > >(plus)(1, 2);
        BOOST_CHECK(x == 3);
    }
    {
        int x = egg::retby<boost::add_const<boost::mpl::_1> >(plus)(1, 2);
        BOOST_CHECK(x == 3);
    }
    {
        char x = egg::retby< nullary_meta< ::add_const > >(always_a)();
        BOOST_CHECK(x == 'a');
    }
}
