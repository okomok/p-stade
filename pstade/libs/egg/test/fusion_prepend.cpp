...
{
    {
        ...
        int v = 999;
        BOOST_CHECK( egg::is_same(v, egg::fusion_at_c<0>(fusion_prepend(t, v))) );
        BOOST_CHECK( !egg::is_same(v, egg::fusion_at_c<0>(X_fusion_prepend<by_value>()(t, v))) ); // copied
        BOOST_CHECK( egg::is_same(boost::get<0>(t), egg::fusion_at_c<1>(fusion_prepend(t, v))) );
        BOOST_CHECK( !egg::is_same(boost::get<1>(t), egg::fusion_at_c<2>(fusion_prepend(t, v))) ); // copied without Fusion.
    }
...
#if defined(PSTADE_EGG_HAS_FUSIONS)
    {
        ...
        BOOST_CHECK( egg::is_same(v, egg::fusion_at_c<0>(fusion_prepend(t, v))) );
        BOOST_CHECK( !egg::is_same(v, egg::fusion_at_c<0>(X_fusion_prepend<by_value>()(t, v))) ); // copied
        BOOST_CHECK( egg::is_same(boost::fusion::at_c<0>(t), egg::fusion_at_c<1>(fusion_prepend(t, v))) );
        BOOST_CHECK( egg::is_same(boost::fusion::at_c<1>(t), egg::fusion_at_c<2>(fusion_prepend(t, v))) ); // not copied with Fusion.
    }
#endif
}
